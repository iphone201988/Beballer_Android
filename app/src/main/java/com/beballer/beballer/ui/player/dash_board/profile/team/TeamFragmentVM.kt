...
@HiltViewModel
class TeamFragmentVM @Inject constructor(private val apiHelper: ApiHelper):BaseViewModel(){
    ...
    fun getTeamListAPi(url: String,data :HashMap<String, Any>) {
        CoroutineScope(Dispatchers.IO).launch {
            commonObserver.postValue(Resource.loading(null))
            try {
                apiHelper.apiGetOnlyAuthToken(url,data).let {
                    if (it.isSuccessful) {
                        commonObserver.postValue(Resource.success("getTeamListAPi", it.body()))
                    } else commonObserver.postValue(
                        Resource.error(handleErrorResponse(it.errorBody(),it.code()), null)
                    )
                }
            } catch (e: Exception) {
                Log.d("error", "getTeamListAPi: $e")
            }
        }
    }
