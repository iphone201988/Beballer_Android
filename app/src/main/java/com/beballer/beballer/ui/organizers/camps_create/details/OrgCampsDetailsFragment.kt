package com.beballer.beballer.ui.organizers.camps_create.details
...
import com.beballer.beballer.data.model.AddTournamentModel
import com.beballer.beballer.data.model.MpvModel
import com.beballer.beballer.databinding.FragmentOrgCampsDetailsBinding
import com.beballer.beballer.databinding.RvAddCampsItemBinding
import com.beballer.beballer.databinding.RvCampsEventSponsorItemBinding
...
@AndroidEntryPoint
class OrgCampsDetailsFragment : BaseFragment<FragmentOrgCampsDetailsBinding>(), OnMapReadyCallback {
    private val viewModel: OrgCampsDetailsFragmentVM by viewModels()
    private lateinit var sponsorsAdapter: SimpleRecyclerViewAdapter<MpvModel, RvCampsEventSponsorItemBinding>
    private lateinit var addTournamentAdapter: SimpleRecyclerViewAdapter<AddTournamentModel, RvAddCampsItemBinding>
    private val translationYaxis = -100F
    private var isFabMenuVisible = false
    private val interpolator = OvershootInterpolator()

    override fun getLayoutResource(): Int {
        return R.layout.fragment_org_camps_details
    }
    ...
    override fun onCreateView(view: View) {
        ...
        binding.tvClassmates.typeface = ResourcesCompat.getFont(requireContext(), R.font.inter_medium)
    ...
    /** handle event sponsors adapter **/
    private fun initSponsorsAdapter() {
        sponsorsAdapter =
            SimpleRecyclerViewAdapter(R.layout.rv_camps_event_sponsor_item, BR.bean) { v, m, pos ->
                when (v.id) {

                }
            }
        sponsorsAdapter.list = getListSponsors()
        binding.rvEvent.adapter = sponsorsAdapter
    }


    /** handle add Tournament adapter **/
    private fun initAddTournamentAdapter() {
        addTournamentAdapter =
            SimpleRecyclerViewAdapter(R.layout.rv_add_camps_item, BR.bean) { v, m, pos ->
                when (v.id) {

                }
            }
        addTournamentAdapter.list = getListAddTournament()
        binding.rvTournaments.adapter = addTournamentAdapter
    }


    // add List in data
    private fun getListSponsors(): ArrayList<MpvModel> {
        return arrayListOf(
    ...
    // add List in data
    private fun getListAddTournament(): ArrayList<AddTournamentModel> {
        return arrayListOf(
            AddTournamentModel("Emending 1"),
            AddTournamentModel("Emending 2"),
            AddTournamentModel(""),
            AddTournamentModel(""),
